...
import { Trash2, Palette, Pin, MapPin, Edit3, Check, Undo2, Image, Upload, Link, Link2, ChevronUp, ChevronDown, ChevronRight } from 'lucide-svelte';
...
let {
	...
	const linkedProjects = $derived(projectsStore.getLinkedProjects(project.id));

	function handleTogglePin() {
		projectsStore.togglePin(project.id);
	}

	function handleToggleLinkProject(targetProjectId: string) {
		const isLinked = project.linkedProjectIds?.includes(targetProjectId);
		if (isLinked) {
			projectsStore.unlinkProjects(project.id, targetProjectId);
		} else {
			projectsStore.linkProjects(project.id, targetProjectId);
		}
	}
...
>
	<!-- Pin Badge -->
	{#if project.isPinned}
		<div class="absolute -right-1 -top-1 rounded-full bg-pastel-peach p-1.5 shadow-md z-10">
	...
	<div class="flex gap-1.5 mt-3 pt-3 px-4 pb-4 border-t border-white/30 opacity-0 transition-opacity group-hover:opacity-100 flex-shrink-0">
		<button
			...
			class="flex-1 flex items-center justify-center gap-1 rounded-md p-1.5 transition {project.isPinned ? 'bg-pastel-peach hover:bg-pastel-peach/80' : 'bg-white/60 hover:bg-white/80'}"
			title={project.isPinned ? 'Unpin project' : 'Pin project'}
		>
			<Pin class="w-4 h-4 text-gray-700" />
		</button>

		<div class="relative flex-1">
			<button
				type="button"
				onclick={() => (showLinkMenu = !showLinkMenu)}
			...
			{#if showLinkMenu}
				<div
					class="absolute bottom-full left-0 mb-2 p-3 bg-white rounded-lg shadow-xl z-50 border border-gray-200 min-w-[200px]"
				>
					<h4 class="text-xs font-semibold text-gray-600 mb-2">Link Projects</h4>
					<div class="space-y-1 max-h-48 overflow-y-auto">
						{#if otherProjects.length > 0}
							{#each otherProjects as otherProject}
								<button
									onclick={() => handleToggleLinkProject(otherProject.id)}
									class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 rounded flex items-center gap-2 transition"
								>
									<div class="w-4 h-4 rounded border-2 border-gray-400 flex items-center justify-center {project.linkedProjectIds?.includes(otherProject.id) ? 'bg-pastel-mint' : 'bg-white'}">
										{#if project.linkedProjectIds?.includes(otherProject.id)}
											<span class="text-xs">✓</span>
										{/if}
									</div>
									<div class="w-3 h-3 rounded-full" style="background-color: {otherProject.color};"></div>
									<span class="flex-1 truncate">{otherProject.name}</span>
								</button>
							{/each}
						{:else}
							<p class="text-xs text-gray-500 px-3 py-2">No other projects</p>
						{/if}
					</div>
				</div>
			{/if}
